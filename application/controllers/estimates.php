	...
	function index()
	{
		...
		if($this->user->admin == 0){ 
			...
			$thisUserHasNoCompanies = (array) $this->user->companies;
				...
				$options = array('conditions' => array('estimate != ? AND company_id in (?)',0,$comp_array));
			...
			if($this->user->admin != 1){
				...
				$thisUserHasNoCompanies = (array) $this->user->companies;
					...
					foreach ($this->user->companies as $value) {
						...
						$this->view_data['estimates'] = Invoice::find('all', array('conditions' => array('company_id in (?)', $comp_array)));
