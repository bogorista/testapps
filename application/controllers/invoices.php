	...
	function index()
	{
		...
		if($this->user->admin == 0){ 
			...
			$thisUserHasNoCompanies = (array) $this->user->companies;
				...
				$options = array('conditions' => array('estimate != ? AND company_id in (?)',1,$comp_array));
		...
		$this->view_data['invoices_due_this_month'] = Invoice::count(array('conditions' => 'UNIX_TIMESTAMP(`due_date`) <= '.$lastday_in_month.' and UNIX_TIMESTAMP(`due_date`) >= '.$firstday_in_month.' AND estimate != 1 AND status != "paid" AND status != "canceled"'));
...
GROUP BY paid_date");
	...
	function view($id = FALSE)
	{
		...
		if($this->user->admin != 1){
			...
			if(!in_array($this->view_data['invoice']->company_id, $comp_array)){redirect('invoices');}
		}
